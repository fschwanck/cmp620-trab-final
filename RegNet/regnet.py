...
class RegNet(pl.LightningModule):
    def __init__(self, regulated_block:nn.Module, in_dim:int, h_dim:int, intermediate_channels:int,
        ...
        self.val_accuracy = tm.Accuracy('multiclass', num_classes=classes)
        self.test_accuracy = tm.Accuracy('multiclass', num_classes=classes)
        self.train_accuracy = tm.Accuracy('multiclass', num_classes=classes)
...
def train_regnet(model, num_epochs=10, num_gpus=1):


    cfm = Cifar10DataModule('./dataset',batch_size=batch_size, download=False)
